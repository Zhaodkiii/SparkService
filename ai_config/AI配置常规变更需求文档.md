...
| 工单号 | 工单名 | 状态 | 范围 |
| --- | --- | --- | --- |
| `AI-CONFIG-000001` | 中国大陆用户才展示 Pro 试用申请入口 | 新需求/待实现 | 客户端 `AITrialSettingsView` 展示条件、`SparkSystemInfo` 统一国家标识判断 |
| `AI-CONFIG-000002` | 试用申请提交后请求通知权限并通过通知刷新 Pro 模型 | 新需求/待实现 | 客户端提交申请、通知权限、公共通知接收、刷新服务端 Pro 模型列表 |
| `AI-CONFIG-000003` | 服务端试用申请延迟自动审核与 APNs 通知 | 新需求/待实现 | `TrialApplication` 申请次数、延迟自动通过、通过/拒绝触发公共 APNs 通知 |
