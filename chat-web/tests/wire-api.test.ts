...
describe("Spark P1 wire adapters", () => {
  ...
  it("normalizes the server's flattened sync Block into the canonical Web Block", () => {
    expect(normalizeSyncBlock({ id: "block-1", kind: "text", status: "ready", revision: 2, order_key: 1000, node_role: "content", text: "server text", content_type: "text/markdown" })).toMatchObject({
      id: "block-1",
      revision: 2,
      payload: { text: "server text", content_type: "text/markdown" },
    });
  });
  ...
  it("sends Run idempotency key and encoded thread/run paths", async () => {
    ...
    await api.create("thread/one", { client_message_id: "msg-1", content: "hi", capability: "chat", references: [], attachments: [], client: { platform: "web", version: "0.1.0", device_id: "device-1" } }, "intent-1");
