...
`建议演进`：新增独立 `chat-web` 登录态对话 App，参考 DeepTutor Chat Workspace 的前端结构与样式，通过服务端 AI Run 和现有聊天同步与移动端共用会话。
...
- 初始化独立 `chat-web`，先完成手机号 OTP、Apple ID、Token 刷新/退出、App Shell、会话侧栏和静态消息/Composer 的 DeepTutor 风格对齐。
...
- [ ] 独立 `chat-web` 当前尚未创建，需按 Web App 工单完成工程、视觉、协议、测试与部署验收。
