...
# CHAT-WEB-027 W1: default classification thresholds for the DeferredFinalAnswerBuffer.
# A round's leading text is buffered, unclassified, until either threshold trips; whichever
# comes first promotes the buffer to a real-time final answer stream via on_final_chunk.
...
async def run_agentic_round(
    ...
    *,
    ...
    - If a ``tool_call_delta`` arrives first, the buffer is classified as
      narration and flushed through ``on_narration_delta`` (existing behavior).
    - If neither threshold trips before a tool call appears, the buffer is
      classified as the final answer and flushed through ``on_final_chunk``;
      every subsequent ``text_delta`` streams straight through in real time.

    Once classified as final, we never roll back already-published text. If a
    ``tool_call_delta`` still arrives afterwards (a rare misclassification), it
    is dropped and logged rather than retracting public text.
    """
    ...
    async for chunk in gateway.stream(
        ...
        # Confirm narration before classifying this chunk's text, so a chunk that
        # carries both a text delta and the first tool_call_delta never races into
        # being misclassified as the final answer.
